...
fun CallScreen(
    ...
    // For outgoing calls: hasLeftIdle starts false, prevents firing on the initial IDLE
    // state before startOutgoingCall() sets OUTGOING_RINGING.
    // For incoming calls: hasLeftIdle starts true, the screen only opens because
    // INCOMING_RINGING fired; if the caller cancels before the first recomposition the
    // only observed state would be IDLE, and we still need onCallEnded() to run.
    var hasLeftIdle by remember { mutableStateOf(isIncoming) }
    ...
    // triggers onCallEnded() (which performs the full session wipe).
    BackHandler {
        when (uiState.callState) {
            CallState.IDLE    -> onCallEnded()                // Already ended, navigate away now
            CallState.INCOMING_RINGING -> viewModel.rejectCall() // Send REJECT to peer
            else              -> viewModel.hangup()            // Cancel outgoing ring or active call
        }
    }
