...
class SplashViewModel @Inject constructor() : ViewModel() {
    ...
    private fun initializeIdentity() {
        // Identity is generated fresh at login time (AuthViewModel.enter).
        // Nothing to initialize here: just signal that the splash is done.
        ...
        try {
            ...
            val message = Message(
                ...
                expiryDeadlineMs = if (header.expirySeconds > 0) {
                    // Floor the deadline to at least 5 s from receipt time to absorb clock skew.
                    val senderDeadline = header.timestampMs + (header.expirySeconds * 1000L)
                    maxOf(senderDeadline, System.currentTimeMillis() + 5_000L)
                } else null,
                isOutgoing = false,
            ...
            try {
                if (isEphemeralMode) {
                    ...
                    conversationId?.let { convId ->
                        // Notify the peer to also wipe, then clean up locally
                        try { backgroundConnectionManager.sendForceLogoutSignal(convId) } catch (_: Exception) {}
                        backgroundConnectionManager.unregisterChatHandler(convId)
                    }
                }
            } finally {
                // Always emit ForceLogout so the UI navigates away, even if cleanup throws
                _events.emit(ChatEvent.ForceLogout)
            }
        }
    }
    ...
    fun initIncomingCall(contactIdOrEphemeralId: String) {
        ...
        viewModelScope.launch {
            ...
            val actualContactId = if (contactIdOrEphemeralId.endsWith("_ephemeral")) {
                ...
                // Look up the actual contact ID via ephemeral session manager
                ephemeralSessionManager.getContactId(sessionUUID) ?: return@launch
            } else {
                contactIdOrEphemeralId
            }
