        ...
        Box(modifier = Modifier.fillMaxSize().padding(padding)) {
            if (showManualEntry) {
                Column(
                    ...
                    Text(
                        "Paste the cryptika:// code shared by your contact.",
...
fun ContactConfirmScreen(
    ...
    val (publicKeyBytes, identityHash, fingerprintHex) = remember(publicKeyB64) {
        ...
        val idHash = com.cryptika.messenger.domain.crypto.IdentityHash.compute(pubKey)
...
fun SettingsScreen(
    ...
    onNavigateToContactConfirm: (String) -> Unit = {},
    onForceLogout: () -> Unit = {},
    viewModel: SettingsViewModel = hiltViewModel(),
    qrScanViewModel: QrScanViewModel = hiltViewModel()
) {
    val state by viewModel.uiState.collectAsState()
    val scanState by qrScanViewModel.scanState.collectAsState()
    ...
    // Handle successful QR code parse → navigate to contact confirmation
    LaunchedEffect(scanState) {
        when (val s = scanState) {
            is ScanState.Success -> {
                val pubKeyB64 = android.util.Base64.encodeToString(
                    s.result.publicKeyBytes,
                    android.util.Base64.URL_SAFE or android.util.Base64.NO_PADDING or android.util.Base64.NO_WRAP
                )
                onNavigateToContactConfirm(pubKeyB64)
                qrScanViewModel.reset()
            }
            else -> {}
        }
    }
    ...
    Scaffold(
        ...
        bottomBar = {
            // SecureKeyboard docked at the bottom: appears when manual entry is active
        ...
        Column(
            ...
            ListItem(
                ...
                trailingContent = {
                    Icon(
                        if (showManualQrEntry) Icons.Filled.KeyboardArrowUp else Icons.Filled.KeyboardArrowDown,
                        contentDescription = null
                    )
                },
                modifier = Modifier.clickable {
                    showManualQrEntry = !showManualQrEntry
                    if (!showManualQrEntry) {
            ...
            if (showManualQrEntry) {
                Column(
                    ...
                    Button(
                        onClick = {
                            val code = manualQrCode.trim()
                            if (code.isNotEmpty()) {
                                qrScanViewModel.processQrResult(code)
                            }
                        },
                        modifier = Modifier.fillMaxWidth(),
