...
class BackgroundConnectionManager @Inject constructor(
    ...
    companion object {
        ...
        const val FORCE_LOGOUT_MAGIC: Byte = 0x04
    }
     ...
     */
    @Volatile var forceLogoutCallback: (() -> Unit)? = null
    ...
    fun getConnectionState(convId: String): StateFlow<ConnectionState>? {
        return convStates[convId]?.connectionState
    }
     ...
     */
    suspend fun sendForceLogoutSignal(convId: String): Boolean {
        val packet = byteArrayOf(FORCE_LOGOUT_MAGIC)
        return sendPacket(convId, "force_logout_${UUID.randomUUID()}", packet)
    }
    ...
    private suspend fun connectConversation(state: ConvState) {
        try {
            ...
            // Initiator (lower identity hash) builds and signs the 76-byte ticket payload,
            // then requests a server countersignature. The acceptor skips this HTTP call;
            // it receives the dual-signed ticket embedded in the initiator's DH offer (type 0x02).
            val amInitiator = state.myIdentity.identityHex < state.contact.identityHex
            if (amInitiator) {
                try {
                    val sorted    = listOf(state.myIdentity.identityHex, state.contact.identityHex).sorted()
                    val aIdBytes  = sorted[0].hexToByteArray()
                    val bIdBytes  = sorted[1].hexToByteArray()
                    val timestamp = System.currentTimeMillis()
                    val expiry    = TicketManager.DEFAULT_EXPIRY_SECONDS

                    // Build the 76-byte payload: a_id(32) || b_id(32) || timestamp_ms(8) || expiry_s(4)
                    val payload = java.nio.ByteBuffer.allocate(76).apply {
                        put(aIdBytes); put(bIdBytes); putLong(timestamp); putInt(expiry)
                    }.array()

                    // Sign SHA-256(payload) with our Ed25519 identity key
                    val payloadHash = java.security.MessageDigest.getInstance("SHA-256").digest(payload)
                    val userASig    = identityKeyManager.sign(payloadHash)

                    // Request server countersignature
                    val ticketResponse = relayApi.requestTicket(
                        ...
                        request = TicketRequest(
                            ...
                            user_a_sig_b64 = Base64.getEncoder().encodeToString(userASig)
                        )
                    )

                    // Verify the returned 204-byte dual-signed ticket
                    val ticketBytes = Base64.getDecoder().decode(ticketResponse.ticket_b64)
                    val userAPubKey = identityKeyManager.getPublicKeyBytes()
                    val verified    = ticketManager.verifyTicket(ticketBytes, userAPubKey)
            ...
            // Acceptor: no HTTP call; ticket arrives embedded in the initiator's DH offer.
                ...
                state.wsClient.events.collect { event ->
                    when (event) {
                        is RelayEvent.Connected -> {
                            ...
                            // Send our DH handshake offer (initiator embeds ticket in type-0x02 offer)
                            val (offerPacket, ephemeralPair) = withContext(Dispatchers.Default) {
                                handshakeManager.createOffer(state.verifiedTicket)
                            }
                        ...
                        is RelayEvent.MessageReceived -> {
                            if (event.message.conversationId == state.conversationId) {
                                ...
                                when {
                                    ...
                                    packetBytes.size == 1 &&
                                    packetBytes[0] == FORCE_LOGOUT_MAGIC &&
                                    state.messageProcessor != null -> {
                                        // Only honour FORCE_LOGOUT after the DH handshake has
                                        // completed, so an unauthenticated relay message cannot
                                        // trigger a wipe before a session is established.
                                        forceLogoutCallback?.invoke()
                                    }
                                    else -> {
    ...
    private suspend fun completeHandshake(state: ConvState, offerBytes: ByteArray) {
        ...
        // Acceptor: if the peer sent a type-0x02 offer, extract and verify the embedded ticket.
        if (state.verifiedTicket == null) {
            try {
                val extracted = handshakeManager.extractTicketFromOffer(
                    offerBytes     = offerBytes,
                    userAPublicKey = state.contact.publicKeyBytes,
                ...
                if (extracted != null) {
                    ...
                    state.verifiedTicket = extracted
                }
            } catch (_: Exception) { }
        }
        ...
        try {
            ...
            // Peer signature invalid; clear ephemeral state — the private key was
            // already zeroized inside computeSharedSecret, so retaining the pair
            // would cause an all-zero DH secret on the next reconnect attempt.
            state.ourEphemeralPair = null
            state.verifiedTicket  = null
        } catch (_: Exception) {
            state.ourEphemeralPair = null
            state.verifiedTicket  = null
        }
        ...
        try {
            ...
            val message = Message(
                ...
                expiryDeadlineMs = if (header.expirySeconds > 0) {
                    // Floor the deadline to at least 5 s from receipt time so clock skew
                    // between sender and receiver never causes instant deletion.
